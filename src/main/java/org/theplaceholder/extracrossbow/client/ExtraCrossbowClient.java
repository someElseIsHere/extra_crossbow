...
public class ExtraCrossbowClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ModelPredicateProviderRegistry.register(Items.CROSSBOW, ExtraCrossbow.id("projectile"), (UnClampedModelPredicateProvider) (stack, world, entity, seed) -> {
            ...
            if (!chargedProjectilesComponent.getProjectiles().isEmpty() && Projectiles.isProjectile(chargedProjectilesComponent.getProjectiles().getFirst().getItem())){
                return Projectiles.getProjectile(chargedProjectilesComponent.getProjectiles().getFirst().getItem()).getTexture();
            } else {
                return 0;
            }
        });
